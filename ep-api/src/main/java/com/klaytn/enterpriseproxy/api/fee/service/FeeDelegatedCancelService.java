...
import com.klaytn.enterpriseproxy.fee.delegated.Cancel;
...
@Service
public class FeeDelegatedCancelService {
    @Autowired
     ...
     * @return
     */
    public ApiResponse cancel(HttpServletRequest request,int chainId,String rawTx) {
        ...
        TransactionResponse response = cancel.cancel(params);
     ...
     * @return
     */
    public ApiResponse cancelWithRatio(HttpServletRequest request,int chainId,String rawTx) {
        if (ObjectUtil.isEmpty(chainId) || StringUtil.isEmpty(rawTx)) {
            return ApiUtils.onError(ApiResponseCode.PARAMETER_INVALID);
        }

        FeeDelegatedParams params = FeeUtil.setFeeDelegatedParams(chainId,rawTx,ApiUtils.getTargetHost(request));
        TransactionResponse response = cancel.cancelWithRatio(params);
